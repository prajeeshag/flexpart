...
C      write(*,*) "convect"
C      write(*,*) "ND:", ND, "NL:", NL, "DELT:", DELT
C      write(*,*) "IFLAG:", IFLAG
C      write(*,*) "PRECIP:", PRECIP
C      write(*,*) "WD:", WD
C      write(*,*) "TPRIME:", TPRIME
C      write(*,*) "QPRIME:", QPRIME
C      write(*,*) "CBMF:", CBMF
...
C
      CBMFOLD=CBMF
c *** C. Forster: adjustment of CBMF is not allowed to depend on FLEXPART timestep
      DELT0=DELT/3.
...
C
      integer i, j, nsb, nst
C
      real (kind=8) ah0, ahg, alv, cl, cpd, cpinv, cpp, cpv, cpvmcl, denom, epsi
      real (kind=8) eps0, es, qg, rd, rg, rv, s, tc, tg
C
